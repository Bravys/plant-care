...
const plants = {
  ...
  ml: ["മാങ്ങ","ചക്ക","ചെമ്പരത്തി","റോസ്","തേങ്ങ","വാഴ","ആഞ്ഞിലി ചക്ക 	","മുല്ല","ഇക്സോറ"]
};
...
// Common categories people search for, with a test run against each plant's data
const categories = {
  en: [
    { label: "🍈 Fruit",     test: d => d.speciality.toLowerCase().includes("fruit") },
    { label: "🌸 Flower",    test: d => d.speciality.toLowerCase().includes("flower") },
    { label: "💊 Medicinal", test: d => d.speciality.toLowerCase().includes("medicinal") },
    { label: "🎍 Ornamental",test: d => d.speciality.toLowerCase().includes("ornamental") },
    { label: "💧 Daily Watering",  test: d => d.water.toLowerCase() === "daily" },
    { label: "🗓️ Weekly Watering", test: d => d.water.toLowerCase() === "weekly" }
  ],
  ml: [
    { label: "🍈 പഴം",     test: d => d.speciality.includes("പഴം") },
    { label: "🌸 പൂവ്",    test: d => d.speciality.includes("പൂവ്") },
    { label: "💊 ഔഷധം",   test: d => d.speciality.includes("ഔഷധ") },
    { label: "🎍 അലങ്കാരം", test: d => d.speciality.includes("അലങ്കാര") },
    { label: "💧 ദിവസേന നനയ്ക്കൽ",     test: d => d.water === "ദിവസേന" },
    { label: "🗓️ ആഴ്ചയിൽ നനയ്ക്കൽ",   test: d => d.water === "ആഴ്ചയിൽ ഒരിക്കൽ" }
  ]
};

// Render the tappable category pills (Fruit, Flower, Daily Watering, etc.)
...
// Filter plants by a tapped category and show matches
function filterByCategory(cat) {
  const matches = Object.entries(plantData[lang])
    .filter(([, data]) => cat.test(data))
    .map(([name]) => name);

  if (matches.length > 0) {
    showResults(matches, cat.label);
  } else {
    alert(lang === "en" ? "No plants found in this category." : "ഈ വിഭാഗത്തിൽ ചെടികളൊന്നും കണ്ടെത്തിയില്ല.");
  }
}
...
// Show multiple search results (from a text search or a category tag)
function showResults(matches, heading) {
  const btnContainer = document.getElementById("buttons");
  const title = heading
    ? (lang === "en" ? `Results: ${heading}` : `ഫലങ്ങൾ: ${heading}`)
    : (lang === "en" ? "Matching plants:" : "പോരുത്തമുള്ള ചെടികൾ:");

  btnContainer.innerHTML = `<h3 id="resultsHeading">${title}</h3>`;
  ...
  resetBtn.innerText = lang === "en" ? "⬅ Show all plants" : "⬅ എല്ലാ ചെടികളും കാണിക്കുക";
  resetBtn.onclick = () => {
